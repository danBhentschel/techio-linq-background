...
namespace Background1
{
    ...
    public class UnitTest
    {
        ...
        {
            Utils.TechioMessage("About to test new List<int> { 10, 20, 30 }.Transform(x => x / 10)");
            var enumerable = new List<int> { 10, 20, 30 }.Transform(x => x / 10);
            var answer = string.Join(" ", enumerable);
            Utils.AssertAreEqual("1 2 3", answer, "{ 10, 20, 30 } -- x / 3");

            Utils.TechioMessage(string.Empty);
            Utils.TechioMessage("About to test new List<int> { 7, 11, 15, 19 }.Transform(x => x - 7)");
            ...
            Utils.TechioMessage(string.Empty);
            Utils.TechioMessage("About to test new List<int> { 1, 1, 1, 2, 2, 2 }.Transform(x => 20)");
            enumerable = new List<int> { 1, 1, 1, 2, 2, 2 }.Transform(x => 20);
            answer = string.Join(" ", enumerable);
            Utils.AssertAreEqual("20 20 20 20 20 20", answer, "{ 1, 1, 1, 2, 2, 2 } -- 20");

            Utils.TechioMessage(string.Empty);
            Utils.TechioMessage("About to test new List<int> { 1, 2, 3 }.Transform(x => x * 2)");
